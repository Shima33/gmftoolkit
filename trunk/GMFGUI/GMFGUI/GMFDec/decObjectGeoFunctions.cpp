...
void readCollisionMesh(int preTabNum)
{
	...
	if (databytes[5] != '\x00')
	{
		...
		myprintf("Decompiling GMID_COLLISION_MESH %s...\n", objName);
		free(objName);
	}
	else
	{
		...
		myprintf("Decompiling GMID_COLLISION_MESH...\n");
	}
	free(databytes);
...
void readObjectGeo(int preTabNum)
{
	...
	if (databytes[5] != '\x00')
	{
		char* objName = getString();
		tab(preTabNum+1); fprintf(output, "*NODE_NAME\t%s\n", objName);	
		myprintf("Decompiling GEOMOBJECT %s...\n", objName);
		free(objName);
	}
	else
	{
		free(getBytes(4));
		tab(preTabNum+1); fprintf(output, "*NODE_NAME\t(null)\n");
		myprintf("Decompiling GEOMOBJECT...\n");
	}
	free(databytes);

	databytes = getBytesNF(6);
	if (databytes[5] != '\x00' && !isRA1)
	{
